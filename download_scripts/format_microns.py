...
vert_positions = M.vertices[['x_nm', 'z_nm']]#[['L' not in ct for ct in M.vertices['cell_type']]]
center = vert_positions.mean(axis=0)
distances = np.linalg.norm(vert_positions.values-center.values, axis=1)
COLUMN_RADIUS = 250
...
).melt(value_name='horizontal', ignore_index=False).reset_index().set_index(['pregid', 'postgid'])['horizontal']
...
connected = pd.Series(0, index=horizontal.index)
...
pairs = pd.DataFrame({
    ...
    'pre_type': M.vertices.loc[pre, 'cell_type'],
    'post_type': M.vertices.loc[post, 'cell_type']
})
...
grouped = pairs.groupby(['pre_type', 'post_type', 'vertical_bin', 'horizontal_bin'])

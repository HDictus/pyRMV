from analysis_neuro import Analysis, terms, measurements
...
def test_requests_measurements():
    ana = Analysis(
        ...
        observations=pd.DataFrame({
            'a param': [1, 2, 3, 4, 5],
            'another param': [4, 4, 4, 3, 3]}))

    models = MockModel(1), MockModel(2)
    report = ana(*models)
    
    expected = pd.concat([models[0].thing(ana.parameters).assign(dataset='1'),
...
def test_runs_plotter():
    ...
    pd.testing.assert_series_equal(
        ...
        pd.Series([100, 200, 300, 400, 500, 4, 4, 4, 4, 4],
                              name='measured thing'))
    pd.testing.assert_series_equal(
...
def test_plots_with_multiple_params():
    ...
    pd.testing.assert_series_equal(
        kwargs['x'].reset_index(drop=True),
        pd.Series(['L1 NGC', 'L23 NGC', 'L4 MC', 'L5 MC', 'L6 LBC'] * 2,
                    name='layer, mtype'))
    pd.testing.assert_series_equal(
        kwargs['hue'].reset_index(drop=True),
        pd.Series(['blabla'] * 5 + ['4'] * 5,
